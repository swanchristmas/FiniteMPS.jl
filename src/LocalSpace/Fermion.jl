...
Four operators of triplet pairing correlation `Δₜ^dag⋅Δₜ`, where `Δₜ` is the triplet pairing operator that carries `2` charge and `1` spin quantum numbers. Rank = `(3, 4, 4, 3)`. Note `(i, j, k, l)` gives `c_i^dag c_j^dag c_l c_k` in the triplet channel.
...
Two rank-`3` operators of `C+C-` correlation where `C+ = c↑^dag c↓^dag` and `C- = C+^dag = c↓c↑`. Note both operators are bosonic.
	 ...
	 SBSB::NTuple{4, TensorMap}
Correlation between spin bond operators `Bᵢⱼ = c_iα^dag (s+ / √2 , sz, s- / √2)_{αβ} c_β`, which is a 3-component spinor. Rank = `(3, 4, 4, 3)`. The overall coefficient is determined by making `(i, i, j, j)` give `Sᵢ⋅Sⱼ`.
"""
module U₁SU₂Fermion
...
const SBSB = let A = FdagF[1], F = FFdag[1]
	...
	@tensor B[d a; b e] := F[a b c] * iso[d c e]
